...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return displayHealth, 0, 2);
	...
	public var client = new SaveVariables();
	public var Hpercent:Float = 50;
	...
	override public function update(elapsed:Float)
	{
		...
		displayHealth = FlxMath.lerp(displayHealth, health, .2/(client.framerate / 60));
		...
		var iconOffset:Int = 50;
		Hpercent = FlxMath.lerp(Hpercent, Math.max(health * 50, 0), elapsed * 10);
		if(Hpercent > 100) {
			Hpercent = 100;
		}
		...
		iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(Hpercent, 0, 100, 100, 0) * 0.01)) + (150 * iconP1.scale.x - 150) / 2 - iconOffset;
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(Hpercent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2.scale.x) / 2 - iconOffset;
		...
		for (name in achievesToCheck) {
			...
			if (name != WeekData.getWeekFileName() + '_nomiss') // common achievements
			{
				switch(name)
				{
					...
					case 'ur_good':
						unlock = (ratingPercent >= 0.9 && !usedPractice);
